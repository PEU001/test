...
# mb_rating_tag.py (final script, will overwrite placeholder after rebuild)
...
def process_file(path: str, root: str, ua: str,
    ...
    result = {
        'file': rel, 'status': 'skip', 'mbid': None, 'rating': None, 'votes': None,
    ...
    try:
        ...
        if not value_votes or value_votes[0] is None:
            msg=f'Aucune note pour MBID {mbid}'; write_log('not-found', rel, msg)
            result.update({'status':'not-found','message':msg}); return result
...
def main():
    p=argparse.ArgumentParser(description='MusicBrainz rating + log + report + cleanup + backup/restore + cache')
    p.add_argument('path')
    ...
    target=args.path
    try:
        for path in iter_audio_files(target):
            res=process_file(path, target, args.ua, args.write_popm, args.search_fallback, args.dry_run,
