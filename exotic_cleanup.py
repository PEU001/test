...
class MbCache:
    """Cache SQLite : ratings (MBID->(rating,votes)) et search mapping (artist|title|dur -> MBID)."""

    def __init__(self, db_path: str, mode: str = "rw", ttl: int = 86400):
        self.db_path = db_path
        self.mode = mode  # ro | rw | refresh
        self.ttl = ttl
    ...
    def key(artist, title, duration_ms):
        ...
        d = int(round((duration_ms or 0) / 1000))
        return f"{a}|{t}|{d}"
    ...
    def get_rating(self, mbid: str):
        if self.mode == "refresh":
            return None
        c = self.conn.cursor()
        c.execute("SELECT rating, votes, fetched_at FROM ratings WHERE mbid=?;", (mbid,))
        r = c.fetchone()
        if not r:
            return None
        rating, votes, ts = r
        if self.mode != "ro" and (time.time() - ts) > self.ttl:
            return None
        return rating, votes
    ...
    def get_search_mbid(self, artist, title, duration_ms):
        ...
        c = self.conn.cursor()
        c.execute("SELECT mbid, fetched_at FROM search_map WHERE qkey=?;", (q,))
        r = c.fetchone()
        if not r:
            return None
        mbid, ts = r
