...
']):
            return '"'+s.replace('"','""')+'"'
        return s
    csv_lines=["file,status,mbid,rating,votes,artist,title,duration,has_cover,exotic_tags,removed_exotic,message"]
    for r in results:
        csv_lines.append(','.join([
            esc(r.get('file')), esc(r.get('status')), esc(r.get('mbid')),
            esc(r.get('rating')), esc(r.get('votes')),
            esc(r.get('artist')), esc(r.get('title')),
            esc(fmt_ms(r.get('duration_ms'))), esc('yes' if r.get('has_cover') else 'no'),
            esc(';'.join(r.get('exotic_tags') or [])), esc(';'.join(r.get('removed_exotic') or [])), esc(r.get('message'))
        ]))
...
'.join(csv_lines).encode('utf-8')).decode('ascii')
    ...
    def h(s):
        import html; return html.escape('' if s is None else str(s))
...
<style>
:root {{ color-scheme: light dark; }} body {{ font-family: system-ui, -apple-system, Segoe UI, Roboto, Arial, sans-serif; margin:24px; }}
...
<div class='toolbar'>
  ...
  data:text/csv;base64,{csv_b64}Exporter CSV</a>
</div>
<table id='tbl'>
  <thead><tr>
...
<script>(function(){const q=document.getElementById('q');const rows=Array.from(document.querySelectorAll('#tbl tbody tr'));function n(s){return (s||'').toLowerCase()}function f(){const qq=n(q.value);rows.forEach(tr=>{const ok=!qq||n(tr.innerText).includes(qq);tr.style.display=ok?'':'none';});}q.addEventListener('input',f);}())</script>
</body></html>"""
