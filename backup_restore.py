...
def backup_tags(audio, path: str, rel: str, backup_dir: str):
    """Sauvegarde tous les tags (et pochettes) en JSON."""
    ...
    # MP4/M4A
        ...
        for k, v in (audio.tags or {}).items():
            if k == "covr":
                ...
                for c in v:
                    covers.append(
                        {"data": base64.b64encode(bytes(c)).decode("ascii"), "type": c.imageformat}
                    )
                data["tags"]["covr"] = covers
            else:
                try:
                    data["tags"][k] = [
                        vv.decode("utf-8", "ignore") if isinstance(vv, bytes) else str(vv) for vv in v
                    ]
                except Exception:
                    data["tags"][k] = str(v)
...
def restore_tags(path: str, rel: str, backup_dir: str):
    """Restaure tous les tags depuis backup JSON."""
    ...
    if fmt == "MP3":
        ...
        for k, v in tags.items():
            if k == "APIC":
                ...
                desc = k.split(":", 1)[1]
                id3.add(TXXX(encoding=3, desc=desc, text=v))
            else:
                cls = Frames.get(k)
                if cls:
                    try:
        ...
        return True, "Tags restaurés (MP3)"
        ...
        for k, v in tags.items():
            if k == "__PICTURES__":
                ...
                for p in v:
                    ...
                    pic.type = p["type"]
                    pic.desc = p["desc"]
                    pic.data = base64.b64decode(p["data"])
                    try:
                ...
                audio[k] = v
        audio.save()
        return True, "Tags restaurés (Vorbis)"

    # MP4
    if fmt == "MP4":
        if not isinstance(audio, MP4):
            return False, "Format incompatible (non MP4)"
        try:
            audio.delete()
        except Exception:
            pass
        for k, v in tags.items():
            if k == "covr":
                covs = []
                for c in v:
                    covs.append(MP4Cover(base64.b64decode(c["data"]), c["type"]))
                audio.tags["covr"] = covs
            else:
                audio.tags[k] = v
        audio.save()
        return True, "Tags restaurés (MP4)"
